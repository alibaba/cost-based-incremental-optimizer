...
import static org.apache.calcite.rel.tvr.utils.TvrRelOptUtils.JoinConditionSplitRes;
...
public class LoptMultiJoin {
  ...
  public void setFactorWeights() {
    ...
    for (RexNode joinFilter : allJoinFilters) {
      ...
      if (factorRefs.cardinality() == 2) {
        ...
        final List<Integer> list = factorRefs.toList();
        for (int outer : list) {
          for (int inner : list) {
            if (outer != inner) {
  ...
  public List<RexNode> getAllJoinFilters() {
    return allJoinFilters;
  }
  ...
  public void addRemovableSelfJoinPair(int factor1, int factor2) {
    ...
    Map<Integer, Integer> leftFactorColMapping = new HashMap<Integer, Integer>();
  ...
  public Edge createEdge(RexNode condition, JoinConditionSplitRes resolvedCondition, JoinRelType type) {
    ImmutableBitSet fieldRefBitmap = fieldBitmap(condition);
    ImmutableBitSet factorRefBitmap = factorBitmap(fieldRefBitmap);
    return new Edge(condition, resolvedCondition, factorRefBitmap,
        fieldRefBitmap, type);
  }

  /**
   * Create cross product edge without conditions.
   *
   * @param factorRefBitmap factor references. In case of cartesian join, the
   * factor references can be any two.
   * @param type join type
   * @return edge.
   */
  public Edge createEdge(ImmutableBitSet factorRefBitmap, JoinRelType type) {
    RexBuilder rexBuilder = multiJoin.getCluster().getRexBuilder();
    RexLiteral truth = rexBuilder.makeLiteral(true);
    JoinConditionSplitRes resolvedJoinConditions = new JoinConditionSplitRes(
        Lists.newArrayList(), Lists.newArrayList(), Lists.newArrayList(),
        Lists.newArrayList());
    return new Edge(truth, resolvedJoinConditions, factorRefBitmap,
        ImmutableBitSet.of(), type);
  }

  /**
   * Information about a join-condition. When resolving the join condition, the
   * smaller factor is as the left RelNode while the bigger factor is as the
   * right RelNode.
   * 
   */
  static class Edge {
    ...
    Edge(RexNode condition, JoinConditionSplitRes resolvedCondition,
        ImmutableBitSet factors, ImmutableBitSet columns, JoinRelType joinType) {
    ...
    void setFactors(ImmutableBitSet factors) {
      this.factors = factors;
    }

    public void reverseJoinType() {
      switch (joinType) {
      case LEFT:
