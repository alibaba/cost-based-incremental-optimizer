...
public class TvrStreamingVirtualSpoolPropRule extends RelOptTvrRule
    implements InterTvrRule {

  public static TvrStreamingVirtualSpoolPropRule INSTANCE =
      new TvrStreamingVirtualSpoolPropRule();

  private TvrStreamingVirtualSpoolPropRule() {
    super(operand(LogicalTvrVirtualSpool.class, tvrEdgeSSMax(tvr()),
   ...
   * <p>
   *
   * LogicalTvrVirtualSpool --SET_SNAPSHOT_MAX--  Tvr1
   *         |                                     QP
   ...
   * <p>
   *                                                      QN
   * LogicalTvrVirtualSpool --SET_SNAPSHOT_MAX--  Tvr1 ------------> Tvr5  --SET_SNAPSHOT_MAX--  newVirtualSpool1
   *         |                                     \   QP                                            |
   *         |                                      ---> Tvr6  --SET_SNAPSHOT_MAX-- newVirtualSpool2 |
   *         |                                                                        |              |
   *         |                                     QP                                 |              |
   *       input      --SET_SNAPSHOT_MAX--  Tvr2 -----> Tvr3  --SET_SNAPSHOT_MAX--  qpInput          |
   *                                          \   QN                                                 |
  ...
  public void onMatch(RelOptRuleCall call) {
    ...
    LogicalTvrVirtualSpool spool = match.get();
    ...
    RelNode newSpool1 =
        spool.copy(spool.getTraitSet(), ImmutableList.of(qnInput));
    RelNode newSpool2 =
        spool.copy(spool.getTraitSet(), ImmutableList.of(qpInput));

    // QP + QN = SetSnapshotMax
    RelNode merge =
        LogicalUnion.create(ImmutableList.of(newSpool1, newSpool2), true);

    call.transformBuilder().addEquiv(merge, spool)
        .addTvrType(merge, rootTvr.getTvrType())
        .addPropertyLink(rootTvr, TvrStreamingPropertyQP.INSTANCE, newSpool2,
            rootTvr.getTvrType())
        .addPropertyLink(rootTvr, TvrStreamingPropertyQN.INSTANCE, newSpool1,
