...
public class VolcanoRuleMatchVisualizer {
  ...
  public void addRuleMatch(String ruleCallID, Collection<? extends RelNode> matchedRels, boolean discardIfNoChange) {
    ...
    boolean tvrChanged = ruleMatchSequence.isEmpty() ||
        ruleTvrInfoMap.get(ruleMatchSequence.get(ruleMatchSequence.size() - 1))
            .equals(tvrs);
    ...
    volcanoPlanner.relImportances.forEach((rel, importance) -> {
      if (importance == 0.0) {
        importanceZeroNodes.add(Integer.toString(rel.getId()));
      }
    });
  ...
  public void writeToFile(String templateDirectory, String outputDirectory, String suffix) {
    try {
      ...
      String htmlTemplate = Resources.toString(Resources.getResource(
          Paths.get(templateDirectory).resolve("viz-template.html").toString()),
          Charsets.UTF_8);
      String tvrHtmlTemplate = Resources.toString(Resources.getResource(
...
// End VolcanoRuleMatchVisualizer.java
